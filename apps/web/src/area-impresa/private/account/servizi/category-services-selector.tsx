...
export type CategoryServiceGroup = {
  groupSlug: string;
  groupName: string;
  serviceIds: string[];
};
...
export type CategoryOption = {
  ...
  /**
   * Service Group (Phase 20.9C) — raggruppamento dei Service di questa
   * Category derivato solo dalla struttura reale Category -> Service
   * (vedi site/services/service-groups.ts), non da MacroArea/SEO. Quando
   * assente o con un solo gruppo, la UI resta piatta come prima (oggi:
   * tutte le Category tranne Impresa edile e Imbianchino). Nessun impatto
   * sul payload salvato: si seleziona sempre per categoryIds/serviceIds.
   */
  serviceGroups?: CategoryServiceGroup[];
};
...
function hasMultipleServiceGroups(category: CategoryOption) {
  return (category.serviceGroups?.length ?? 0) > 1;
}
...
export function CategoryServicesSelector({
  ...
  // groupKey identifica univocamente lo "scope" di ricerca/paginazione di una
  // lista di servizi: la Category stessa (caso piatto, invariato) oppure
  // Category+ServiceGroup (caso raggruppato, solo Impresa edile/Imbianchino
  // oggi).
  function groupKeyFor(categoryId: string, serviceGroupSlug?: string) {
    return serviceGroupSlug ? `${categoryId}::${serviceGroupSlug}` : categoryId;
  }

  function getGroupQuery(groupKey: string) {
    return serviceQueries[groupKey] ?? "";
  }

  function getVisibleServiceCount(groupKey: string) {
    return visibleServiceCounts[groupKey] ?? serviceBatchSize;
  }

  function getFilteredServices(services: ServiceOption[], groupKey: string) {
    const query = getGroupQuery(groupKey);
  ...
  function getVisibleServices(services: ServiceOption[], groupKey: string) {
    const filteredServices = getFilteredServices(services, groupKey);
    ...
    return [
      ...selectedServices,
      ...unselectedServices.slice(0, getVisibleServiceCount(groupKey)),
    ];
  }

  function hasMoreServices(services: ServiceOption[], groupKey: string) {
    const filteredServices = getFilteredServices(services, groupKey);
    const unselectedServices = filteredServices.filter(
      (service) => !selectedServiceIdSet.has(service.id),
    );

    return unselectedServices.length > getVisibleServiceCount(groupKey);
  }

  function getServicesForGroup(
    category: CategoryOption,
    group: CategoryServiceGroup,
  ) {
    const serviceIdSet = new Set(group.serviceIds);

    return category.services.filter((service) => serviceIdSet.has(service.id));
  }

  function getActiveVisibleServices(category: CategoryOption) {
    if (hasMultipleServiceGroups(category)) {
      return (category.serviceGroups ?? []).flatMap((group) =>
        getVisibleServices(
          getServicesForGroup(category, group),
          groupKeyFor(category.id, group.groupSlug),
        ),
      );
    }

    return getVisibleServices(category.services, groupKeyFor(category.id));
  }

  const activeVisibleServices = activeCategory
    ? getActiveVisibleServices(activeCategory)
    : [];
  ...
  function renderServiceGroup({
    ...
    const visibleServices = getVisibleServices(services, groupKey);
    const showServiceSearch = services.length > serviceBatchSize;
    ...
    return (
      <div key={groupKey} className="space-y-3">
        ...
        {hasMoreServices(services, groupKey) ? (
          <Button
            type="button"
            variant="secondary"
            onClick={() => {
              setVisibleServiceCounts((currentCounts) => ({
                ...currentCounts,
                [groupKey]: getVisibleServiceCount(groupKey) + serviceBatchSize,
              }));
            }}
          >
            Mostra altri
        ...
        {selectedCategories.length === 0 ? (
          ...
          <div className="space-y-3">
            {selectedCategories.map((category) => {
              ...
              const groupedByServiceGroup = hasMultipleServiceGroups(category);
              ...
              return (
                ...
                >
                  ...
                  {open ? (
                    <div className="space-y-6 border-t border-border-primary bg-surface-secondary p-4">
                      {category.services.length === 0 ? (
                        <p className="text-sm text-text-muted">
                          Nessun servizio disponibile per questa categoria.
                        </p>
                      ) : groupedByServiceGroup ? (
                        (category.serviceGroups ?? []).map((group) =>
                          renderServiceGroup({
                            services: getServicesForGroup(category, group),
                        ...
                        renderServiceGroup({
                          services: category.services,
                          groupKey: groupKeyFor(category.id),
                        })
                      )}
                    </div>
                  ) : null}

...
const errorMessages: Record<string, string> = {
  invalid_website:
    "Inserisci un URL valido per il sito web.",
  invalid_radius:
    "Seleziona un raggio operativo valido.",
  invalid_coordinates:
...
function redirectWithError(code: string): never {
  redirect(
    `/area-impresa/profilo?error=${encodeURIComponent(code)}`,
  )
}
...
async function updateCompanyProfileAction(
  ...
  const website =
    normalizeWebsite(formData.get("website"))
  const radiusValue = Number(
    normalizeText(
      formData.get("operatingRadiusKm"),
    ),
  )
  const latitude =
    normalizeCoordinate(
      ...
      <section className="space-y-6">
        ...
        <Card className="p-6">
          <div className="border-b border-border-primary pb-5">
            ...
            <p className="mt-2 text-sm leading-6 text-text-secondary">
              Aggiorna sito web, sede operativa e raggio usati per la
              dashboard richieste.
            </p>
          </div>

...
function getRawAnswers(structuredData: unknown): JsonRecord {
  ...
  if (
    isRecord(structuredData) &&
    isRecord(structuredData.rawAnswers)
  ) {
...
function findDescription(
  ...
  if (typeof draft.customerDescription === 'string') {
    const description =
      draft.customerDescription.trim()
...
async function loadRequest({
  ...
  try {
    return {
      ok: true as const,
      request:
        await getCustomerRequestByHistoryToken({
          token,
          requestId,
        }),
    }
...
export default async function CustomerRequestDetailPage({
  ...
  const description =
    result.ok
      ? findDescription(
          result.request.structuredData,
        )
      : null
  const details =
    result.ok
      ? buildStructuredDetails(
          result.request.structuredData,
        )
      : []
  const historyHref =
    buildHistoryHref(token ?? '')

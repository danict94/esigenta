...
export type SendConversationMessageInput = {
  ...
  now?: Date
  /**
   * When the caller has already verified the COMPANY actor (e.g. via requireCompanyActor()),
   * pass it here so the domain runtime skips the redundant getCompanyActorForUser() DB call.
   */

...
1. **Data:** every row from the 6 dropped tables, plus every non-default `Company.requestMatchingMode` value and non-null `RequestDispatch.matchedServiceIds` value, is preserved verbatim in `docs/taxonomy-refoundation/16_pre_drop_backup.json`.
